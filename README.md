...
- поддержка `sensor` battery percentage и `binary_sensor` low-battery
...
- режимы мониторинга `all except excluded` и `include only`
...
Сущность попадает под мониторинг, если выполняется один из вариантов:
...
- домен сущности: `binary_sensor`
- `device_class`: `battery`
- состояние `on` трактуется как low battery
- состояние `off` трактуется как восстановление
...
- included battery sensors
- warning/critical/recovery message templates
...
- `Monitor all except excluded` — мониторить все найденные батарейные сущности, кроме списка исключений
- `Monitor included entities only` — мониторить только сущности из списка включённых

В режиме `include only` список исключений не используется для принятия решения о мониторинге.
...
Для `binary_sensor` значения `{level_percent}` и `{level}` будут пустыми.
...
- отправка проверочного уведомления из `Settings -> Devices & Services -> Battery Informer -> Configure`

### Проверочное уведомление из настроек

В параметрах интеграции есть переключатель:

- `Send notification for the lowest tracked battery when saving`

Если включить его и сохранить настройки, интеграция отправит уведомление по самой разряженной из найденных батарей через тот же `notify` канал, который используется для обычных уведомлений. Это позволяет проверить именно реальный путь доставки, а не отдельный тестовый сервис.

Если отслеживаемые батареи ещё не найдены, форма покажет ошибку и уведомление не будет отправлено.
...
## Как интеграция ищет батарейки

Поиск работает тремя способами одновременно:

1. Стартовый snapshot при запуске интеграции.
2. Обновление по событиям `state_changed`.
3. Периодический полный перескан всех `sensor`.

### Стартовый snapshot

При запуске интеграции выполняется полный проход по `hass.states.async_all("sensor")`.

В список попадают только те сущности, которые:

- находятся в домене `sensor`
- имеют `device_class = battery`
- имеют числовое состояние от `0` до `100`
- имеют единицу измерения `%` или не имеют её

Также попадают `binary_sensor` с `device_class = battery`, где:

- `on` означает low battery
- `off` означает normal

### Обновление по событиям

После стартового snapshot интеграция слушает события изменения состояний.

Если подходящий батарейный сенсор:

- появляется позже
- меняет уровень заряда
- временно становится `unknown` или `unavailable`
- возвращается обратно в валидное состояние

то внутренний список батареек обновляется сразу по событию.

### Периодический полный перескан

Дополнительно интеграция делает регулярный полный перескан всех `sensor`.

Это нужно для случаев, когда:

- какая-то интеграция загрузилась позже
- батарейный сенсор не прислал ожидаемое событие
- при старте Home Assistant сенсор ещё не был готов
- состояние появилось после отдельной перезагрузки другой интеграции

Интервал пересканирования задаётся параметром:
...
- поддержка дополнительных vendor-specific battery атрибутов
- отдельные entity для warning-уровня
- более гибкий рендер шаблонов уведомлений
