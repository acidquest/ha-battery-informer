...
async def async_setup_entry(hass: HomeAssistant, entry: BatteryInformerConfigEntry) -> bool:
    ...
    config = {
        ...
        CONF_INCLUDED_ENTITIES: entry.options.get(CONF_INCLUDED_ENTITIES, entry.data.get(CONF_INCLUDED_ENTITIES, DEFAULT_INCLUDED_ENTITIES)),
        CONF_MONITORING_MODE: entry.options.get(CONF_MONITORING_MODE, entry.data.get(CONF_MONITORING_MODE, DEFAULT_MONITORING_MODE)),
        CONF_RESCAN_INTERVAL_MINUTES: entry.options.get(
            CONF_RESCAN_INTERVAL_MINUTES,
